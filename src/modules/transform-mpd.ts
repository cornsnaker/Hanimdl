...
import { LanguageItem, findLang, languages } from './langsData';
import { console } from './log';
import * as reqModule from './fetch';
import { FetchParams } from './fetch';
...
export async function parse(manifest: string, language?: LanguageItem, url?: string) {
	...
	// Audio Loop
	for (const item of Object.values(parsed.mediaGroups.AUDIO.audio) as any[]) {
		for (const playlist of item.playlists) {
			...
			const pItem: AudioPlayList = {
				...
				segments: playlist.segments.map((segment: any): Segment => {

...
import * as fontsData from './modules/fontsData';
...
import { api } from './modules/api-urls';
import * as reqModule from './modules/fetch';
import { DownloadedMedia, sxItem } from './@types/hidiveTypes';
import parseFileName, { Variable } from './modules/filename';
import { downloaded } from './modules/downloadArchive';
import parseSelect from './modules/parseSelect';
import { AvailableFilenameVars } from './modules/args';
...
import { MPDParsed, parse } from './modules/transform-mpd';
import { canDecrypt, getKeysWVD, cdm, getKeysPRD } from './modules/cdm';
import { FetchParams } from './modules/fetch';
...
export default class Hidive {
			...
			if ((apiReq.error && apiReq.error.res?.status == 401) || (apiReq.res && apiReq.res.status == 401)) {
				...
				if (await this.refreshToken()) {
					...
					if (!apiReq.ok || !apiReq.res) {
						...
						return { ok: false, res: apiReq.res, error: apiReq.error };
					}
				} else {
	...
	public async doAuth(data: AuthData): Promise<AuthResponse> {
		...
		const authReq = await this.apiReq(
			'/v2/login',
			{ id: data.username, secret: data.password },
			'auth'
		);
			...
			const videoDownload = await new streamdl({
				...
				callback: options.callbackMaker
					? options.callbackMaker({
							...
							parent: { title: selectedEpisode.seriesTitle },
	...
	public async muxStreams(data: DownloadedMedia[], options: Record<any, any>, inverseTrackOrder: boolean = true) {
		...
		const merger = new Merger({
			...
			onlyAudio: hasAudioStreams
				? audioItems.map((a): MergerInput => ({ lang: a.lang, path: a.path }))
				: [],
			output: `${options.output}.${options.mp4 ? 'mp4' : 'mkv'}`,
			subtitles: subtitleItems.map((a): SubtitleInput => ({
				file: a.path,
				language: a.language,
				closedCaption: a.cc
			})),
